...
export function Sidebar({
  ...
  return (
    <aside
      ...
      className={`flex flex-col bg-icloud-bg-sidebar border-r border-icloud-border h-full overflow-hidden shrink-0 select-none ${isAnyPaneResizing ? '' : 'transition-all duration-300'} ${isMobile ? (mobileVisible ? 'fixed inset-0 z-[200] w-full bg-icloud-bg-sidebar' : 'hidden') : ''}`}
    ...
    >
      ...
      <header className="px-5 py-4 flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          <button 
            ...
            className="p-1.5 hover:bg-icloud-text-primary/5 dark:hover:bg-icloud-text-primary/10 rounded-md transition-colors"
            title={isSidebarCollapsed ? 'Show Sidebar' : 'Hide Sidebar'}
            aria-label={isSidebarCollapsed ? 'Show sidebar' : 'Hide sidebar'}
          >
             <ChevronLeft className="w-5 h-5 text-icloud-text-tertiary" strokeWidth={1.5} />
          </button>
          <button 
            onClick={onCompose}
            className="p-1.5 hover:bg-icloud-text-primary/5 dark:hover:bg-icloud-text-primary/10 rounded-md transition-colors"
          ...
          >
            <Plus className="w-5 h-5 text-icloud-accent" strokeWidth={2} />
          </button>
        </div>
      </header>
      ...
      <nav aria-label="Mailboxes" className="flex-1 overflow-y-auto px-3 space-y-3 py-2">
        {mailboxesLoading ? (
          ...
          <div className="flex flex-col items-center justify-center py-10 px-4 text-center">
             <Mail className="w-8 h-8 text-icloud-text-secondary mb-2 opacity-40" strokeWidth={1.25} />
            <p className="text-[12px] text-icloud-text-secondary font-medium">No folders loaded</p>
            <button 
              onClick={() => refetchMailboxes()} 
              className="mt-2 text-[12px] text-icloud-accent font-medium hover:underline"
            >
              Retry
            </button>
          </div>
          ...
          <>
            ...
            {customMailboxes.length === 0 && systemMailboxes.length > 0 && (
              <button
                onClick={onCreateFolder}
                className="flex items-center justify-center gap-2 px-3 py-2 text-[13px] font-medium text-icloud-accent hover:bg-icloud-accent/10 rounded-lg transition-colors mt-2"
              >
            ...
            {/* Smart Mailboxes Section */}
              <div className="pt-2 border-t border-icloud-divider">
              <div className="pt-3 pb-2 px-3 text-[10px] font-bold text-icloud-text-secondary uppercase tracking-wider opacity-60">Smart Mailboxes</div>
            ...
            {/* Apps Section */}
            <div className="pt-2 border-t border-icloud-divider">
              <div className="pt-3 pb-2 px-3 text-[10px] font-bold text-icloud-text-secondary uppercase tracking-wider opacity-60">Apps</div>
      ...
      <div className="p-4 border-t border-icloud-divider text-[11px] text-icloud-text-secondary bg-icloud-bg-sidebar space-y-2">
