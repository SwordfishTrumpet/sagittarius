...
export function Sidebar({
  ...
  return (
    <aside
      ...
      className={`flex flex-col bg-[#F2F2F7]/70 dark:bg-[#1C1C1E]/70 backdrop-blur-xl border-r border-[#E5E5EA] dark:border-[#38383A] h-full overflow-hidden shrink-0 select-none ${isAnyPaneResizing ? '' : 'transition-all duration-300'} ${isMobile ? (mobileVisible ? 'fixed inset-0 z-[200] w-full bg-[#F2F2F7] dark:bg-[#1C1C1E]' : 'hidden') : ''}`}
      style={{ width: isMobile ? '100%' : (isSidebarCollapsed ? 0 : sidebarWidth) }}
    >
      {!isSidebarCollapsed && (
      <>
      <header className="px-5 py-4 flex items-center justify-between">
        <h1 className="text-xl font-bold tracking-tight flex items-center leading-none text-[#1C1C1E] dark:text-white">
          sagittarius
        </h1>
        <div className="flex items-center gap-2">
          ...
          >
            <Plus className="w-5 h-5 text-[#007AFF] dark:text-[#0A84FF]" strokeWidth={2} />
          </button>
        </div>
      </header>
      ...
      <nav aria-label="Mailboxes" className="flex-1 overflow-y-auto px-3 space-y-3 py-2">
        {mailboxesLoading ? (
          <div className="flex items-center justify-center py-10 opacity-30">
            <div className="w-5 h-5 border-2 border-[#007AFF] dark:border-[#0A84FF] border-t-transparent rounded-full animate-spin" />
          </div>
          ...
          <div className="flex flex-col items-center justify-center py-10 px-4 text-center">
            ...
            <button 
              ...
              className="mt-2 text-[12px] text-[#007AFF] dark:text-[#0A84FF] font-medium hover:underline"
            >
              Retry
            </button>
          </div>
          ...
          <>
            ...
            {customMailboxes.length === 0 && systemMailboxes.length > 0 && (
              <button
                onClick={onCreateFolder}
                className="flex items-center justify-center gap-2 px-3 py-2 text-[13px] font-medium text-[#007AFF] dark:text-[#0A84FF] hover:bg-[#007AFF]/10 dark:hover:bg-[#0A84FF]/10 rounded-lg transition-colors mt-2"
              >
      ...
      <div className="p-4 border-t border-[#E5E5EA] dark:border-[#38383A] text-[11px] text-[#8E8E93] bg-white/30 dark:bg-black/30 space-y-2">
         {quota && quota.hardLimit && quota.hardLimit > 0 && (
           <QuotaBar used={quota.used || 0} total={quota.hardLimit || 0} />
         )}
         <div className="flex items-center justify-between">
           <div className="flex items-center gap-2">
             <span className="truncate max-w-[100px] font-medium text-[#1C1C1E] dark:text-white">{userLabel || 'User'}</span>
             {isOffline ? (
               <>
               ...
               <Wifi className="w-3 h-3 text-[#30D158]" strokeWidth={2} />
             ) : (
               <WifiOff className="w-3 h-3 text-[#8E8E93]" strokeWidth={2} />
             )}
           </div>
          <div className="flex items-center gap-2">
             <button onClick={onOpenSettings} className="p-1 hover:bg-black/5 dark:hover:bg-white/10 rounded transition-colors" title="Settings" aria-label="Settings">
               <SettingsIcon className="w-3.5 h-3.5 text-[#8E8E93]" strokeWidth={1.5} />
             </button>
            <button onClick={() => jmapClient.logout()} className="text-[#007AFF] dark:text-[#0A84FF] font-medium hover:underline">Sign Out</button>
          </div>
         </div>
       </div>
      </>
