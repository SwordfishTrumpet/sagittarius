...
### Added
- **Server-reachability UI (issue #6)** — booting with a dead backend now shows a full-screen "Mail server unreachable" state (configured hostname, Retry, Sign Out) instead of a misleadingly healthy cached UI; the message list renders an error banner with Retry when the threads query fails with no cache; Settings shows the configured mail server (read-only). The boot identity check now distinguishes "backend host does not resolve" (blocks) from "fingerprint endpoint missing" (fails open, preserving legacy-session behavior).
- **Backend-aware /health (issue #7)** — `/health` probes the configured JMAP backend (DNS + connection, cached 10s) and returns HTTP 503 + `status: "degraded"` with a distinct `backend` field when the backend is down; the Docker HEALTHCHECK and `deploy.sh` now fail exactly when mail is broken. `server.cjs` gained the same `/health` handler (it previously had none).
- **Unified proxy 502 responses (issue #8)** — `server.js`, `server.cjs` and the Vite dev proxy all answer backend-outage failures with the same JSON shape (`{"error":"JMAP backend unavailable"}`) via a shared `writeBadGatewayResponse()`; the dev proxy previously wrote no response at all, leaving clients hanging until timeout. UI error rendering goes through `getUserFacingJMAPErrorMessage()` so users see classified messages (session expired / server unreachable / protocol error) instead of raw strings like `JMAP request failed: 502`.
- **Startup JMAP_SERVER validation (issue #9)** — `server.js` and `server.cjs` probe the configured backend at boot (DNS + connection) and log an enhanced `JMAP backend: … (host, resolved, reachable, addresses)` line; an unresolvable host logs a loud WARNING and `JMAP_FAIL_FAST_ON_UNRESOLVED=1` aborts boot (exit 1). `VITE_LOGIN_EMAIL_DOMAIN` is syntax-validated at build time.
...
- **Unreachable-backend deferral (issue #5)** — mutations that fail with server-unreachable errors (proxy 502/504, DNS failure, refused connections) are now queued to the offline pipeline even when the machine is online, within a bounded 10-minute deferral window (`src/utils/serverReachability.ts`). Sends/moves/deletes during a backend outage appear in the "Pending sync" state and replay automatically when the backend becomes reachable again (first successful request resets the episode and triggers replay). Past the window, new mutations surface the error so users are not queueing silently.

### Changed
- **Integration-test fetch queue hardening (issue #10)** — the whole-App integration helper (`src/test/integration/helpers.tsx`) no longer throws on a request that does not line up with the seeded `respondWith` queue. The queue tolerates reordered and duplicate JMAP requests: each seeded response is consumed once in seed order, `repeatable: true` marks a response sticky (serves every matching request), and unmatched requests reuse the most recent response served for the same request signature (or get a benign 404 for genuinely unknown requests, e.g. identity/BIMI probes) — so query-hook reordering or refetch duplicates under full-suite parallel load can no longer starve later assertions of data. `useAppUpdateChecker` is mocked out in integration tests (its `/index.html` probe is not part of the mail flows under test). CI now runs the full test suite twice to catch flakes.
